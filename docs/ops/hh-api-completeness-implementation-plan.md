...
1. Longer supervised `detail-worker` run for throughput/storage/failure mix.
2. Production alert delivery.
3. Transport/outage hardening and operator-visible resume semantics after the near-complete baseline result.
4. VPS pilot for the next full `search-only` baseline rerun on a more stable host.
5. Serious completeness measurements on several full sweeps with first-detail drain.

Не делать наоборот:

- не запускать long completeness study без measured first-detail drain contour;
...
Достаточный MVP уже есть:

1. persistent first-detail backlog selector
2. dedicated drain command
3. thin `detail_worker` loop
4. operator metrics for backlog size/drain rate/cooldown
5. later integration into scheduler if operationally needed
...
- реализовано как terminal `terminal_404`;
...
- добавлен selector поверх `vacancy_current_state`, не ограниченный `last_seen_run_id`;
- выбираются vacancies без закрытого first-detail outcome;
...
- `hhru_first_detail_ready_backlog_size{scope}`
- `hhru_first_detail_cooldown_backlog_size{scope}`
...
1. Провести более длинный supervised `detail-worker` run локально или на VPS pilot DB.
2. Оформить production alert delivery.
3. Довести transport/status hardening.
4. Затем получить clean terminal `search-only` baseline на VPS.

То есть backlog implementation уже не является следующим blocker-ом; следующий blocker теперь operational validation и production hardening.
