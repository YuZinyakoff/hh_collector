...
- `HHRU_PROMETHEUS_RETENTION_TIME=7d`
- `HHRU_PROMETHEUS_RETENTION_SIZE=8GB`

Эти env-переменные прокидываются в Prometheus как:

- `--storage.tsdb.retention.time=${HHRU_PROMETHEUS_RETENTION_TIME}`
- `--storage.tsdb.retention.size=${HHRU_PROMETHEUS_RETENTION_SIZE}`
...
- first-detail open/ready/cooldown backlog panels;
- `Resume Unresolved Again In 12h`;
- debt-tables по run-ам с failed / unresolved / repair backlog;
- outcome tables для scheduler, resume, detail repair и first-detail drain;
...
- `hhru_run_tree_total_partitions` и `hhru_run_tree_failed_partitions` читаются как тот же lifecycle snapshot, что и coverage ratio: Prometheus хранит последний snapshot каждого `run_type`, а история всех run остаётся в Postgres.
